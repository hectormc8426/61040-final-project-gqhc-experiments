<!-- Renders the ratings for a given lesson within the lesson. Can optionally show user input so they can rate lesson -->

<template>
    <div id="lessonRatingGroup">
        <div v-if="!loading" id="ratingContainer">
            <h2 v-if="letInput">Rate This Lesson</h2>
            <h2 v-else>Ratings For This Lesson</h2>
            <div id="ratingList">
                <div v-for="category in Object.keys(ratings)" id="ratingBlock">
                    ...
                    <CreateRatingForm :contentId="lesson._id" :category="category"
                        :score="Number(user_ratings[category])" :change-score-callback="changeUserScore"
                        v-if="letInput" />
                </div>
            </div>
        </div>
